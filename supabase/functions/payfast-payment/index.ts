...
// Custom MD5 implementation for Deno
const md5 = (text: string): string => {
  // Import from a reliable MD5 library for Deno
  const encoder = new TextEncoder();
  const data = encoder.encode(text);
  
  // Simple MD5 implementation using bit manipulation
...
serve(async (req) => {
  ...
  try {
    ...
    formData.signature = generateSignature(formData, passphrase);
